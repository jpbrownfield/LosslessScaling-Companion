...
from ..core.models import ManagedPackageConfig, Profile
...
class GraphicsResolver:
    ...
    def resolve(
        ...
        for provider, config in (
            ("reshade", graphics.reshade),
            ("special-k", graphics.special_k),
        ):
            package = self._package(provider, config)
            if package:
                files.extend(self._recipe_files(package))
        if (
            profile.reshade
